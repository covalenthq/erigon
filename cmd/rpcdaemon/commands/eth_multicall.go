...
type MulticallRunlist map[libcommon.Address][]hexutil.Bytes

type MulticallPerBlockResult map[libcommon.Address][]*MulticallExecutionResult

type MulticallResultByContract map[libcommon.Address]MulticallPerContractResult
...
func computeWithCachedBalanceSlot(stateReader state.StateReader, contractAddr libcommon.Address, holderAddr []byte) ([]byte, bool) {
	baseSlot, ok := vm.ContractBalanceOfSlotCache.Load(contractAddr)
	if !ok || baseSlot == nil {
		return nil, false
	}
	baseSlotH := baseSlot.(libcommon.Hash)
...
func (api *APIImpl) Multicall(ctx context.Context, commonCallArgs ethapi.CallArgs, queries []MulticallQuery, overrides *map[libcommon.Address]ethapi.Account) ([]MulticallResultByContract, error) {
	startTime := time.Now()
		...
		for _, number := range query.BlockNumbers {
			...
			for contractAddr, payloads := range query.Contracts {
				...
				for _, payload := range payloads {
					if err := libcommon.Stopped(execCtx.Done()); err != nil {
						return nil, err
					}
					...
					blockCtx := transactions.NewEVMBlockContext(api.engine(), blockHeader, blockNrOrHash.RequireCanonical, dbtx, api._blockReader)
					...
					ibs.Prepare(libcommon.Hash{}, blockHeader.Hash(), execSeq)
