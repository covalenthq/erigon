...
func ExecuteBlockEphemerallyForBSC(
	...
	for i, tx := range block.Transactions() {
		...
		if vmConfig.Debug && vmConfig.Tracer == nil {
			vmConfig.Tracer = vm.NewStructLogger(&vm.LogConfig{}, nil) //fateme's note: added nil for encoder
			writeTrace = true
		}
...
func ExecuteBlockEphemerally(
	...
	for i, tx := range block.Transactions() {
		...
		writeTrace := false
		if vmConfig.Debug && vmConfig.Tracer == nil {
			tracer, err := getTracerFn(i, tx.Hash())
			if err != nil {
				return nil, err
			}
		...
		if writeTrace {
			...
			logs := FormatLogs(vmConfig.Tracer.(*vm.StructLogger).StructLogs())
			if err2 := vmConfig.Tracer.(*vm.StructLogger).Encoder().Encode(logs); err2 != nil {
				panic(err2)
			}
			// if err2 := w.Close(); err2 != nil {
			// 	panic(err2)
