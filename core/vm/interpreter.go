...
type balanceOfCachingState struct {
	StorageSlotBase  libcommon.Hash
	HashedStorageKey libcommon.Hash
	LoadedValue      libcommon.Hash
}
...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	if in.Depth() == 1 && bytes.HasPrefix(contract.Input, BALANCEOF_SELECTOR) {
